...
const Home = () => {
    ...
    const handleDelete = async (name: any) => {
        let response = await axios.post('https://product-management-web-app-r7id.onrender.com/remove', { name })
        console.log(response);
        setUse(response as any);
            ...
            <div className='flex flex-wrap items-center justify-center gap-4 mb-4'>
                {
                    Array.isArray(product) && product.map((element: any, index) => (
                        <Card
                            key={index}
                            name={element.name} desc={element.description} categ={element.category} rating={element.rating} price={element.price} url={element.imgURL} func={ handleDelete} />
